...
class FormAreaNone extends PolymerElement
{
	...
	InputElement txtNameSearch;
	InputElement txtCodeSearch;
	SelectElement selTypeSearch;
	...
	txtNameSearch.onKeyUp.listen(handlerOnchageSearch);
	txtCodeSearch.onKeyUp.listen(handlerOnchageSearch);
	selTypeSearch.onChange.listen(handlerOnchageSearch);
	//Load
	init();
	}
	/*
	 ...
	 * @version:1.0
	 */
	void handlerOnchageSearch(Event e)
	{
		Map request=new Map();
		request["code"]=(txtCodeSearch.value != null)?txtCodeSearch.value:"";
		request["name"]=(txtNameSearch.value != null)?txtNameSearch.value:"";
		OptionElement opStatus=new OptionElement();
		opStatus=selTypeSearch.children.elementAt(selTypeSearch.selectedIndex);
		request["status"]=opStatus.value;
		CurrentAreas=filterByValueSearch(listAreas, request);
		Pagination();
	 ...
	 * @version:1.0
	 */
	List<Map> filterByValueSearch(List<Map> locations,Map listValueSearch)
	{
		return locations.where((location)
		{
			if(listValueSearch["status"]!="-1")
			{
				if(location["name"].toLowerCase().contains(listValueSearch["name"].toLowerCase()) && location["code"].toLowerCase().contains(listValueSearch["code"].toLowerCase()) && location["status"]==listValueSearch["status"])
				{
					return true;
				}
			}
			...
			{
				if(location["name"].toLowerCase().contains(listValueSearch["name"].toLowerCase()) && location["code"].toLowerCase().contains(listValueSearch["code"].toLowerCase()))
				{
					return true;
				}
			}
